...
bool Grammar::LL1_parser(const std::vector<Token>& tokens) {
    ...
    while(!parserStack.empty()) {
        ...
        if(sym.nature == Nature::Terminal && sym.type == tokens[index].type) {
            ...
            if(parseTable.find({sym, symbolTable[tokenName[tokens[index].type]]}) == parseTable.end()) return false;

            Production production = parseTable[{sym, symbolTable[tokenName[tokens[index].type]]}];
